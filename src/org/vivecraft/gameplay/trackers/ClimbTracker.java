...
public class ClimbTracker extends Tracker {
    ...
    public boolean isActive(ClientPlayerEntity p) {
        ...
        if (p.isPassenger())
            return false;
        return isClimbeyClimbEquipped() || (p.moveForward == 0 && p.moveStrafing == 0);
    }
    ...
    public void doProcess(ClientPlayerEntity player) {
            ...
            if (!latched[c] && !nope) { //grab
                if (allowed[c]) {
                    ...
                    if ((!wasinblock[c] && inblock[c] && button[c]) ||
                        ...
                        if (!ladder)
                            mc.vrPlayer.blockDust(latchStart[c].x, latchStart[c].y, latchStart[c].z, 5, bp, bs, 0.1f, 0.2f);
        ...
        if ((latched[0] || latched[1]) && rand.nextInt(20) == 10) {
            ...
            BlockState bs = mc.world.getBlockState(bp);
            if (!ladder)
                mc.vrPlayer.blockDust(latchStart[latchStartController].x, latchStart[latchStartController].y, latchStart[latchStartController].z, 1, bp, bs, 0.1f, 0.2f);
        }


        Vector3d now = mc.vrPlayer.vrdata_world_pre.getController(latchStartController).getPosition();
        Vector3d start = OpenVRPlayer.room_to_world_pos(latchStart_room[latchStartController], mc.vrPlayer.vrdata_world_pre);
