...
//		return target<cap ? target : cap;
    ...
    public void doProcess(ClientPlayerEntity player) {
        ...
        boolean main = isHoldingBow(player, Hand.MAIN_HAND);
        ...
        if (isDrawing && !pressed && lastpressed && getDrawPercent() > 0.0) {
            ...
            CCustomPayloadPacket pack = NetworkHelper.getVivecraftClientPacket(PacketDiscriminators.DRAW, ByteBuffer.allocate(4).putFloat(getDrawPercent()).array());
